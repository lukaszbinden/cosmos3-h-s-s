...
# 15-second autoregressive Arm-A-versus-Arm-C comparison.
...
run_group() {
    ...
    mkdir -p "$out" "$runtime"
    echo "AR15_GROUP_START condition=$EVAL_CONDITION subset=$subset gpu=$gpu episodes=$* time=$(date -Is)"
    (
    ...
    echo "AR15_GROUP_DONE condition=$EVAL_CONDITION subset=$subset gpu=$gpu time=$(date -Is)"
}
...
for result_path in sorted(root.glob("raw/*/c3hss_results.json")):
    ...
    if payload["max_chunks"] != 13 or payload["output_frames"] != 150:
        raise SystemExit(f"{result_path} has the wrong rollout horizon")
    records.extend(payload["results"])
if len(records) != 10:
    raise SystemExit(f"expected 10 results, found {len(records)}")
bad = [record["episode_id"] for record in records if record["num_frames"] != 150]
if bad:
    raise SystemExit(f"episodes with non-150-frame rollouts: {bad}")
print("AR15_CONDITION_DONE results=10 frames_each=150 duration_seconds=15.0")
PY
