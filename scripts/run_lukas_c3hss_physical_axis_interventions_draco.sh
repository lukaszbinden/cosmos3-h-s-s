#!/usr/bin/env bash
# Container entrypoint for physically valid per-axis dVRK action interventions.
...
if [[ "$status" -ne 0 ]]; then
    echo "FATAL: at least one physical-axis intervention group failed" >&2
    exit "$status"
fi

echo "===== dVRK PHYSICAL-AXIS INTERVENTIONS COMPLETE $(date -Is) ====="
