...
class NormMonitor(Callback):
    ...
    def _compute_l2_stats(self, tensor: torch.Tensor, detach: bool = True) -> dict[str, torch.Tensor]:
        ...
        if isinstance(data, DTensor):
            data = data.to_local()
        if data.numel() == 0:
            # Under N-way FSDP sharding, a parameter's LOCAL shard can be empty on
            # some ranks (param smaller than world_size, or uneven split). The stock
            # callback then calls data.abs().max() on a 0-element tensor and raises
            # "max(): Expected reduction dim ... numel() == 0" at iter 0
            # on_before_optimizer_step (job 5523421, ranks 43-47). An empty shard
            # contributes zero locally; the cross-rank all_reduce(SUM/MAX) still
            # gives the correct global stat. (Mirrors the sean-cosmos3_surgical_fd
            # overlay fix.)
