...
def _require_openh_stats_postfix(dataset_path) -> str:
    """Return the stats postfix, HARD-FAILING if unset.

    Stats live in the shared dataset ``meta/`` dirs, so reading the bare
    ``stats_cosmos.json`` / ``stats_cosmos-44D.json`` could silently pick up a
    DIFFERENT experiment's stats (e.g. a colleague's 54D run) and corrupt
    normalization. We therefore require ``COSMOS_OPENH_STATS_POSTFIX`` to be set
    for Open-H / CMR embodiments. Set ``COSMOS_OPENH_ALLOW_BARE_STATS=1`` to
    intentionally allow the bare names (legacy/baseline).
    """
    postfix = _get_openh_stats_postfix()
    if postfix:
        return postfix
    if os.environ.get(COSMOS_OPENH_ALLOW_BARE_STATS_ENV, "").strip() in ("1", "true", "True"):
        print(
            f"{_get_rank_prefix()}WARNING: {COSMOS_OPENH_STATS_POSTFIX_ENV} is unset and "
            f"{COSMOS_OPENH_ALLOW_BARE_STATS_ENV} is set — using the BARE stats filename, which "
            f"can collide with another experiment's stats in the shared meta/ dir."
        )
        return ""
    raise RuntimeError(
        f"\n{'=' * 80}\n"
        f"{COSMOS_OPENH_STATS_POSTFIX_ENV} IS NOT SET\n"
        f"{'=' * 80}\n"
        f"Dataset: {dataset_path}\n\n"
        f"Open-H / CMR stats live INSIDE the shared dataset meta/ dirs, so the bare\n"
        f"stats_cosmos.json / stats_cosmos-44D.json names would risk reading ANOTHER\n"
        f"experiment's stats (e.g. a colleague's 54D run) and silently corrupting\n"
        f"normalization. Export the SAME postfix the stats were computed with, e.g.:\n"
        f"    export {COSMOS_OPENH_STATS_POSTFIX_ENV}=c3hss-v1\n\n"
        f"(Intentionally want the bare name? set {COSMOS_OPENH_ALLOW_BARE_STATS_ENV}=1.)\n"
        f"{'=' * 80}"
    )
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if "agibot" in str(stats_path):
            ...
            # experiments writing into the same shared meta/ dir; REQUIRED (hard-fail
            # if unset, unless COSMOS_OPENH_ALLOW_BARE_STATS=1); strict-match.
            _postfix = _require_openh_stats_postfix(self.dataset_path)
            ...
            # collisions in the shared meta/ dir; REQUIRED (hard-fail if unset,
            # unless COSMOS_OPENH_ALLOW_BARE_STATS=1); strict-match.
            _postfix = _require_openh_stats_postfix(self.dataset_path)
