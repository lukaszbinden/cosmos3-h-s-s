...
# ``BasePolicy`` lives in the external ``gr00t_dreams`` package which is NOT
# bundled with Cosmos3.  This eval helper is dead code in the Cosmos3 training
# path (it's only used by the action-policy notebook + standalone eval
# scripts in the original Cosmos-Predict2.5 surgical fork).  If you need
# to revive it, ``pip install gr00t_dreams`` or port the ``model.policy``
# module into Cosmos3 too.  We guard the import so this file still imports
# cleanly without ``gr00t_dreams`` installed — the eval functions below
# will simply fail at call time with a clearer NoneType error.
try:
    from gr00t_dreams.model.policy import BasePolicy  # type: ignore[import-not-found]
...
def calc_mse_for_single_trajectory(
    ...
    assert state_joints_across_time.shape == gt_action_joints_across_time.shape == pred_action_joints_across_time.shape
