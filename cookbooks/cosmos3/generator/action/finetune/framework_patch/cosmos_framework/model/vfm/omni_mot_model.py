...
class OmniMoTModel(ImaginaireModel):
        ...
        if self.config.action_gen and x0_action is not None and len(x0_action) > 0:
            ...
            for i in range(len(xt_action)):
                if gen_data_clean.raw_action_dim is not None and gen_data_clean.raw_action_dim[i] is not None:
                    # CAMP overlay (Phase 3b): prepended memory-code rows carry a
                    # DENSE learned code across all channels — exempt them from
                    # the per-sample padded-channel zeroing. 0 when absent
                    # (pristine upstream behavior for non-CAMP samples).
                    _camp_mem_rows = 0
                    if gen_data_clean.num_memory_action_rows is not None and gen_data_clean.num_memory_action_rows[i] is not None:
                        _camp_mem_rows = int(gen_data_clean.num_memory_action_rows[i])
                    xt_action[i][_camp_mem_rows:, gen_data_clean.raw_action_dim[i] :] = 0
    ...
    def _prepare_inference_data(
        ...
        if has_action:
            ...
            for i, (x0_action, cond_mask_action) in enumerate(
                ...
                if gen_data_clean.raw_action_dim is not None and gen_data_clean.raw_action_dim[i] is not None:
                    # CAMP overlay (Phase 3b): see the noising-site note — memory
                    # rows are conditioning here (noise_action_i = x0 = the code
                    # on those rows) and must not be channel-truncated.
                    _camp_mem_rows = 0
                    if gen_data_clean.num_memory_action_rows is not None and gen_data_clean.num_memory_action_rows[i] is not None:
                        _camp_mem_rows = int(gen_data_clean.num_memory_action_rows[i])
                    noise_action_i[_camp_mem_rows:, gen_data_clean.raw_action_dim[i] :] = 0
    ...
    def _slice_gen_data_clean(self, gen_data_clean: GenerationDataClean, start: int, limit: int) -> GenerationDataClean:
        ...
        if has_action:
            ...
            # CAMP overlay (Phase 3b): slice alongside raw_action_dim.
            num_memory_action_rows = gen_data_clean.num_memory_action_rows[start:limit] if gen_data_clean.num_memory_action_rows else None
        else:
        ...
        return GenerationDataClean(
            ...
            raw_action_dim=raw_action_dim,
            num_memory_action_rows=num_memory_action_rows,
    ...
    def get_data_and_condition(self, data_batch: dict[str, torch.Tensor], iteration: int = 1) -> GenerationDataClean:
        ...
        # CAMP overlay (Phase 3b): per-sample prepended memory-row counts,
        # emitted by CampActionTransformPipeline; absent for non-CAMP runs.
        num_memory_action_rows = data_batch.get("num_memory_action_rows", None)
