...
class GenerationDataClean:
    ...
    # CAMP overlay (Phase 3b): per-sample count of prepended memory-code rows.
    # These rows carry a DENSE learned code across all 44 channels, so the
    # per-sample padded-channel zeroing (raw_action_dim) must skip them.
    # None / absent = no memory rows = pristine upstream behavior.
    num_memory_action_rows: list[torch.Tensor] | None = None
